 ...
 * @mobile 18711832023
 */
class FeedDecoration(var mContext:Context, var mData:MutableList<FeedRecord>): ItemDecoration() {
    ...
    override fun getItemOffsets(outRect: Rect,view: View,parent: RecyclerView,state: RecyclerView.State) {
        ...
        if (position==0&&position<mData.size){
            ...
            if (mData[position].feedTime!=mData[position-1].feedTime){
                outRect.set(0, titleHeight, 0, 0);
            }else{
                outRect.set(0, 0, 0, 0);
            }
    ...
    override fun onDraw(c: Canvas, parent: RecyclerView, state: RecyclerView.State) {
        ...
        for (i in 0 until childCount){
            ...
            position -= 1  //SRecyclerView默认加了一个头viewHolder 所以计算时 position要减1
            if (position==0&&position<mData.size){
                c.drawText(mData[position].feedTime?:"", ScreenUtils.dip2px(mContext,16f).toFloat(), (child.top-ScreenUtils.dip2px(mContext,30f)).toFloat(), mPaint!!)
            }else if (position>0&&position<mData.size){
                if (mData[position].feedTime!=mData[position-1].feedTime){
                    c.drawText(mData[position].feedTime?:"", ScreenUtils.dip2px(mContext,16f).toFloat(), (child.top-ScreenUtils.dip2px(mContext,30f)).toFloat(), mPaint!!)
                }
