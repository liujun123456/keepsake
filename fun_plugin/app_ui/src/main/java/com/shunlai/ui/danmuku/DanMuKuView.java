 ...
 * @mobile 18711832023
 */
public class DanMuKuView extends View implements IDanmakuView, IDanmakuViewController {
    ...
    protected volatile DanMuKuHandler handler;
    ...
    public DanMuKuView(Context context, AttributeSet attrs, int defStyle) {
    ...
    private synchronized void stopDraw() {
        ...
        DanMuKuHandler handler = this.handler;
        this.handler = null;
        unlockCanvasAndPost();
        if (handler != null) {
            handler.quit();
    ...
    private void prepare() {
        if (handler == null)
            handler = new DanMuKuHandler(getLooper(mDrawingThreadType), this, mDanmakuVisible);
    }
    ...
    private Runnable mResumeRunnable = new Runnable() {
        @Override
        public void run() {
            DanMuKuHandler drawHandler = handler;
            if (drawHandler == null) {
                return;
            }
            mResumeTryCount++;
            if (mResumeTryCount > 4 || DanMuKuView.super.isShown()) {
                drawHandler.resume();
            } else {
