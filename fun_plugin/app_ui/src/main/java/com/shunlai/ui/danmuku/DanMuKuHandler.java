 ...
 * @mobile 18711832023
 */
public class DanMuKuHandler extends Handler {
    private DanmakuContext mContext;
    private DanMuKuHandler.FrameCallback mFrameCallback;
    ...
    public DanMuKuHandler(Looper looper, IDanmakuViewController view, boolean danmakuVisibile) {
        super(looper);
    ...
    public void handleMessage(Message msg) {
        ...
        switch (what) {
            ...
            case RESUME:
                removeMessages(DanMuKuHandler.PAUSE);
                quitFlag = false;
                if (mReady) {
            ...
            case PAUSE:
                removeMessages(DanMuKuHandler.RESUME);
    ...
    private void initRenderingConfigs() {
        ...
        if (getConfig() instanceof DanMuKuContext){
            mFrameUpdateRate= ((DanMuKuContext)getConfig()).getFrameUpdateRate();
        }else {
    ...
    public void seekTo(Long ms) {
        ...
        removeMessages(DanMuKuHandler.UPDATE);
        removeMessages(DanMuKuHandler.RESUME);
        removeMessages(DanMuKuHandler.SEEK_POS);
        obtainMessage(DanMuKuHandler.SEEK_POS, ms).sendToTarget();
    }
    ...
    public void resume() {
        removeMessages(DanMuKuHandler.PAUSE);
        sendEmptyMessage(DanMuKuHandler.RESUME);
    }
    ...
    public void prepare() {
        ...
        if (mContext.updateMethod == 0) {
            mFrameCallback = new DanMuKuHandler.FrameCallback();
        }
        mUpdateInSeparateThread = (mContext.updateMethod == 1);
        sendEmptyMessage(DanMuKuHandler.PREPARE);
    }

    public void pause() {
        removeMessages(DanMuKuHandler.RESUME);
        syncTimerIfNeeded();
        sendEmptyMessage(DanMuKuHandler.PAUSE);
    }
